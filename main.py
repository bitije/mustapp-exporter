...
class MustappSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [f'https://mustapp.com/@{self.username}/', ]
        for url in urls:
            ...
            for movie_id in movie_list[1].split(','):
                if movie_list[0] == 'want':
                    ...
                    yield response.follow(
                        movie_url, callback=self.parse_want_movie_page)
                elif movie_list[0] == 'watched':
                    movie_url = f'https://mustapp.com/@{self.username}/{movie_id}/'
                    yield response.follow(
                        movie_url, callback=self.parse_watched_movie_page)

    def parse_want_movie_page(self, response):
    ...
    def parse_watched_movie_page(self, response):
